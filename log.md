...
- Добавлен старт этапа 2 в `python/segmentation.py`: разбиение на верхнюю и нижнюю панели, raw/clean-маски сигнала и overlay для проверки линий.
- Проект перезапущен через `.venv/bin/python python/main.py`.
...
- Проект повторно запущен через `.venv/bin/python python/main.py`.
...
- В `python/segmentation.py` усилена `clean_signal_mask()`: добавлена фильтрация компонентов по площади, ширине, высоте, удаление узких высоких артефактов у правого края и мелких компонентов в верхней зоне панели.
...
- В `python/main.py` добавлено сохранение CSV с пиксельными рядами `x_px,y_px`.
- Проект перезапущен через `.venv/bin/python python/main.py`, debug-файлы пересохранены.
...
- В `python/segmentation.py` добавлен режим `panel_type` для `analyze_panel(panel_image, panel_type)` и `clean_signal_mask(mask, panel_type)`.
- В `python/main.py` вызовы разделены на `analyze_panel(upper_panel, "upper")` и `analyze_panel(lower_panel, "lower")`.
...
- Проект перезапущен через `.venv/bin/python python/main.py`, debug-файлы и CSV пересохранены.
...
- Проверка `.venv/bin/python -m py_compile python/main.py python/segmentation.py` прошла успешно.
- Финальная калибровка по-прежнему не выполнялась.

## 2026-05-08 16:03:45 MSK — Этап 2: устойчивые маски и трассировка

- Этап 1 не изменялся: `python/preprocess.py`, параметры Hough и red mask для выравнивания не трогались.
- В `python/segmentation.py` переработана компонентная фильтрация этапа 2: добавлены общие правила для мелкого шума, маленьких квадратных компонент и тонких широких горизонтальных элементов.
...
- В `python/main.py` добавлен вывод метрик этапа 2: компоненты до/после, удаленные компоненты, raw/interpolated points, большие разрывы и coverage по ширине.
- Проект перезапущен через `.venv/bin/python python/main.py`; debug-файлы и CSV пересохранены.
...
- Этап 1 не изменялся: `python/preprocess.py`, выравнивание, Hough и red mask не трогались.
...
- В `python/segmentation.py` добавлен `cleanup_mode` для lower strict/soft очистки.
...
- В `python/main.py` добавлено сохранение `lower_clean_mask_strict.png`, `lower_clean_mask_soft.png`, `lower_signal_overlay_strict.png`, `lower_signal_overlay_soft.png`.
...
## 2026-05-12 21:25:58 MSK — Подготовка к этапу 3

- Этап 1 не изменялся: `python/preprocess.py` не трогался.
- Алгоритмы очистки этапа 2 в `python/segmentation.py` не изменялись.
- В `python/main.py` убраны дублирующиеся финальные сообщения; оставлен короткий вывод о завершении этапов 1 и 2 и готовности к этапу 3.
- Создан `python/calibration.py` как каркас для будущей калибровки пиксельных координат в физические значения.
- В `calibration.py` добавлены функции `load_points_csv()`, `calibrate_time_axis()`, `calibrate_fhr_axis()`, `calibrate_ua_axis()` и `build_final_csv()`.
- Калибровочные точки пока заданы ручными константами-заглушками; автоматическое распознавание шкал не добавлялось.
- Добавлены TODO-комментарии для подбора x-координат временных меток, y-координат шкалы FHR и y-координат шкалы UA.
- Проект перезапущен через `.venv/bin/python python/main.py`; этапы 1 и 2 работают как раньше.
...
- Проверка `.venv/bin/python -m py_compile python/main.py python/segmentation.py python/calibration.py` прошла успешно.
...
- Добавлен `data/requirements.txt` с зависимостями `numpy`, `opencv-python`, `matplotlib`.
