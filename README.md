...
`python_impl/` — полная reference implementation: preprocess/alignment, segmentation/extraction и calibration/result.csv. `cpp_impl/` сейчас покрывает этапы 1 и 2: alignment и segmentation/extraction; C++-калибровка пока не реализована.
...
C++-часть сейчас покрывает первые два этапа: загрузку `data/input.jpg` или `data/input.png`, выделение сетки, Hough-поиск горизонталей, расчет угла, разбиение на верхнюю/нижнюю панели, очистку масок сигнала, overlay и сохранение пиксельных рядов `x_px,y_px` в `results/cpp/debug/`.
...
## Проверенный порядок полного запуска
...
- Реализовать C++ calibration по Python-эталону.
